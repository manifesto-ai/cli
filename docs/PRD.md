...
이건 구조적으로는 옳다. 다만 사용자 입장에서는 "codegen은 선택적 주입", "skills 설치는 에이전트별 별도 명령(`manifesto-skills install-claude`, `install-codex` 등)", "향후 studio/lsp는 또 별도 설정"으로 느껴진다. 특히 skills는 명시적 설치 모델을 택하고 있어 자동 설치보다 안전하지만, 그만큼 "놓치기 쉬운 단계"가 된다.
...
- skills는 설치했지만 필요한 에이전트 후처리(`install-claude`, `install-codex` 등)가 안 됨
- 패키지 버전 간 호환성이 어긋남
...
✓ @manifesto-ai/skills installed.
  ...
  Next steps depend on your tooling:
    ...
    All agents:    pnpm exec manifesto-skills install-all
```

이 설계의 근거: Constitution에서 skills는 "postinstall이 아니라 explicit setup"을 택했다. CLI는 이 explicit 모델을 유지한 채, 사용자가 선택한 에이전트 installer를 그대로 실행하는 조정면으로 동작해야 한다.
...
manifesto doctor
  ...
  Skills
    ⚠ @manifesto-ai/skills installed, but requested agent setup not detected
      → Run: pnpm exec manifesto-skills install-claude
